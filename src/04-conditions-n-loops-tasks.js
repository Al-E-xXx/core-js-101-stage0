...
function findFirstSingleChar(str) {
  ...
  return Object.keys(charCount).find((key) => charCount[key] === 1) || null;
  // throw new Error('Not implemented');
}
...
function isCreditCardNumber(ccn) {
  const ccnString = ccn.toString().trim();
  let sum = 0;

  const parity = ccnString.length % 2;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    if (
      strArr[i] in brackets &&
      i + 1 < strArr.length &&
      brackets[strArr[i]] === strArr[i + 1]
    ) {
      strArr.splice(i, 2);
      return isBracketsBalanced(strArr.join(''));
    }
  }
